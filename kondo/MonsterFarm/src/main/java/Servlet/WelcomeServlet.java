...
public class WelcomeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("hiddenだよ" + hidden);
		 ...
		 String users_id = "sample_ishide508";
		 String users_password = "sample_ishide508";
	...
	//	   int    monsterparam_mp = 100;
		...
		if(userId.isEmpty() || pass.isEmpty() ) {
			if(hidden.equals("login")) {
				...
				session1.setAttribute("error","空欄やめろや" );	
				...
				HttpSession session1 = request.getSession();
				session1.setAttribute("error","空欄やめろや" );	
			   	RequestDispatcher dispatcher1 = request.getRequestDispatcher("shinki.jsp");
			    dispatcher1.forward(request, response);;
			}
		}
		...
		if(idLen > 6  || passLen > 6) {
			if(hidden.equals("login")) {
				HttpSession session1 = request.getSession();
				session1.setAttribute("error","文字多い" );	
				RequestDispatcher dispatcher1 = request.getRequestDispatcher("index.jsp");
			    dispatcher1.forward(request, response);;
	...
	//	System.out.println(flag);
		  ...
		  if(hidden.equals("login")) {
			  HttpSession session1 = request.getSession();
			  session1.setAttribute("error","正しくない認証です" );	
		...
		System.out.println("############ログイン1#############");
		UserDAO uD = new UserDAO();
		System.out.println("############ログイン2#############");
		//jspからjsに値受け渡す
		...
		// users
		jsonMap_users.put("id" ,userId);
		jsonMap_users.put("password" , pass);

		System.out.println("############ログイン201#############");
